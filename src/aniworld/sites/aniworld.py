"""
AniWorld.to site module.

Provides search, listing, homepage parsing, and episode/season counting
for aniworld.to. Uses the AJAX JSON API for search and HTML scraping
for homepage content and episode data.
"""
...
# ---------------------------------------------------------------------------
# Season / Episode counting for aniworld.to
# ---------------------------------------------------------------------------
...
def _parse_season_episodes(soup: BeautifulSoup, season: int) -> int:
    """Parse episode count for a specific season from aniworld.to HTML."""
...
def _parse_episode_titles(soup: BeautifulSoup, season: int) -> Dict[int, str]:
    ...
    # Find all table rows or list items that contain episode links
    for link in soup.find_all("a", href=pattern):
        ...
        # Walk up to the row/container that holds title spans
        row = link.find_parent("tr") or link.find_parent("li") or link.parent

        if row:
            # Try German title span first
...
def get_season_episode_count(slug: str) -> Dict[int, int]:
    """
    Get episode count for each season of an anime on aniworld.to.
    ...
    try:
        ...
        logging.error("Failed to get season episode count for %s on aniworld.to: %s", slug, err)
        return {}
...
def get_movie_episode_count(slug: str) -> int:
    """
    Get movie count for an anime on aniworld.to.
    ...
    try:
        ...
        logging.error("Failed to get movie count for %s on aniworld.to: %s", slug, err)
        return 0
